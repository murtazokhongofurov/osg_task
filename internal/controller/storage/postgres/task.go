...
func (t *TaskRepo) CreateTask(task *m.TaskReq) (*m.TaskRes, error) {
	...
	query := `
	INSERT INTO 
		tasks(developer_id, title, description, file_url, started_date, finished_date, status)
	VALUES
		($1, $2, $3, $4, $5, $6, $7)
	RETURNING 
		id, developer_id, title, description, file_url, started_date, finished_date, status, created_at`
...
func (t *TaskRepo) GetTask(id int) (*m.TaskRes, error) {
	...
	SELECT
		...
		started_date, finished_date, status, created_at, updated_at
	FROM 
		tasks
	WHERE 
		id=$1`
...
func (t *TaskRepo) UpdateTask(task *m.TaskRes) (*m.TaskRes, error) {
	...
	UPDATE 
		tasks
	SET 
		title=$1, description=$2, file_url=$3, started_date=$4, finished_date=$5, updated_at=NOW()
	WHERE
		id=$6
	RETURNING 
		id, developer_id, title, description, file_url, started_date, finished_date, status, created_at, updated_at`
	err := t.db.Pool.QueryRow(context.Background(), query,
